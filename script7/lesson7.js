...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !haveMine(new_unit)) {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood();
        score++;
        var snakePoints = document.querySelector("span.snake-points");
        snakePoints.innerText = "Счёт : " + score;
...
function createMine() {
    // var mineCreated = false;

    // while (!mineCreated) { //пока мину не создали
    // рандом
    ...
    // проверка на змейку
    if (!mine_cell_classes.includes('snake-unit') && !mine_cell_classes.includes('food-unit')) {
        var classes = '';
